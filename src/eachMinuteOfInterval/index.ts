...
export function eachMinuteOfInterval(interval: Interval): ZonedDateTime[] {
  ...
  let current = interval.start.with({ second: 0, millisecond: 0, microsecond: 0, nanosecond: 0 });
  const end = interval.end.with({ second: 0, millisecond: 0, microsecond: 0, nanosecond: 0 });
